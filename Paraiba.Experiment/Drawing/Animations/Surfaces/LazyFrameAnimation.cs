...
namespace Paraiba.Drawing.Animations.Surfaces
{
	...
	public class LazyFrameAnimation : AnimationSurface
	{
		...
		public LazyFrameAnimation(IList<Surface> surfaces, float time, bool isTotalTime)
			: this(surfaces, isTotalTime ? new LazyWrap<float>(() => time / surfaces.Count) : time)
		{
		}

		public LazyFrameAnimation(IEnumerable<Surface> surfaces, Wrap<float> interval)
		{
			Contract.Requires(surfaces != null);

			_surfaces = new LazyWrap<IList<Surface>>(() => surfaces.ToList()).ToListWrap();
			_interval = interval;
		}

		public LazyFrameAnimation(IEnumerable<Surface> surfaces, float time, bool isTotalTime)
		{
			Contract.Requires(surfaces != null);

			_surfaces = new LazyWrap<IList<Surface>>(() => surfaces.ToList()).ToListWrap();
			_interval = isTotalTime ? new LazyWrap<float>(() => time / _surfaces.Count) : time;
		}
