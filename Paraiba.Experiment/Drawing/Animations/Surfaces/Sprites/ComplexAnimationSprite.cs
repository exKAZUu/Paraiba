...
namespace Paraiba.Drawing.Animations.Surfaces.Sprites
{
	...
	public class ComplexAnimationSprite : AnimationSprite
	{
		...
		{
			_animationSprites[_iAnimationSprites].Draw(g, offset, coordinateTransformer);
		}
		...
		public override float ExcessTime
		{
			get
			{
				// 無駄だと思われるが、正確な値を計算する
				float time = 0;
				for (int i = _iAnimationSprites; i < _animationSprites.Count; i++)
		...
		public override bool Elapse(float time)
		{
			...
			while (_animationSprites[_iAnimationSprites].Ended)
			{
				...
				_animationSprites[_iAnimationSprites].Elapse(_animationSprites[_iAnimationSprites - 1].ExcessTime);
