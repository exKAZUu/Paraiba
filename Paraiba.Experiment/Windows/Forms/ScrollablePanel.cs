...
namespace Paraiba.Windows.Forms
{
	public partial class ScrollablePanel : UserControl
	{
		...
		public void Centering(Point2 p)
		{
			...
			ArrangePanelLocation(-(p.X - size.Width / 2), -(p.Y - size.Height / 2));
		}
		...
		private void ArrangePanelLocation(int x, int y)
		{
			...
			if (_hScrollBar.Visible)
			{
				x = XMath.Center(x, -(_hScrollBar.Maximum + 1 - _hScrollBar.LargeChange), 0);
				_hScrollBar.Value = -x;
			}
			else
			{
			...
			if (_vScrollBar.Visible)
			{
				y = XMath.Center(y, -(_vScrollBar.Maximum + 1 - _vScrollBar.LargeChange), 0);
				_vScrollBar.Value = -y;
			}
			else
			{
		...
		private void ArrangeScrollBar()
		{
			...
			{
				hScrollBar.SetBounds(0, Height - hScrollBar.Height, visibleWidth, 0,
			...
			{
				vScrollBar.SetBounds(Width - vScrollBar.Width, 0, 0, visibleHeight,
