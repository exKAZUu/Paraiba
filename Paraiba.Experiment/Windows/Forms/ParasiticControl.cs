...
namespace Paraiba.Windows.Forms
{
	public partial class ParasiticControl : UserControl
	{
		...
		protected Rectangle GetParentRectangle(int x, int y, int width, int height)
		{
		...
		private void ParentControl_MouseWheel(object sender, MouseEventArgs e)
		{
			...
			if (_entered)
			{
				OnMouseWheel(new MouseEventArgs(e.Button, e.Clicks, e.X - Left, e.Y - Top, e.Delta));
			}
		}
		...
		private void ParentControl_MouseClick(object sender, MouseEventArgs e)
		{
			...
			if (_entered)
			{
				OnMouseClick(new MouseEventArgs(e.Button, e.Clicks, e.X - Left, e.Y - Top, e.Delta));
			}
		}
