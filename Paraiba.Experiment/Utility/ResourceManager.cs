...
namespace Paraiba.Utility
{
	...
	{
		protected IDictionary<TKey, LinkedListNode<ResourceItem<TResource>>> ResourceDict;
		protected LinkedList<ResourceItem<TResource>> ResourceList;
		protected IDictionary<TKey, LinkedListNode<ResourceItem<WeakReference>>> WeakResourceDict;
		...
		protected ResourceManager(IComparer<TKey> keyComparer)
		{
			...
			ResourceDict = new SortedDictionary<TKey, LinkedListNode<ResourceItem<TResource>>>(keyComparer);
			WeakResourceDict = new SortedDictionary<TKey, LinkedListNode<ResourceItem<WeakReference>>>(keyComparer);
		}
		...
		protected ResourceManager(IEqualityComparer<TKey> keyComparer)
		{
			...
			ResourceList = new LinkedList<ResourceItem<TResource>>();
			WeakResourceList = new LinkedList<ResourceItem<WeakReference>>();
			ResourceDict = new Dictionary<TKey, LinkedListNode<ResourceItem<TResource>>>(keyComparer);
			WeakResourceDict = new Dictionary<TKey, LinkedListNode<ResourceItem<WeakReference>>>(keyComparer);
		}

		/// <summary>
		/// 指定したノードが保持するリソースを解放する。
		/// 指定したノードはリソースリストから削除して、キャッシュリストに追加して管理する。
		/// </summary>
		...
		protected void Release(LinkedListNode<ResourceItem<TResource>> node)
		{
			...
			var newNode = WeakResourceList.AddFirst(
				new ResourceItem<WeakReference>(key,
					new WeakReference(new DisposableResource(this, key, rc))));
		...
		/// <summary>
		/// 指定したノードが保持するリソースを完全に解放する。
		/// 指定したノードはリソースリストから削除して、DisposeResource メソッドを呼び出す。
		/// </summary>
		/// <param name="node"></param>
		protected void ReleaseCompletely(LinkedListNode<ResourceItem<TResource>> node)
		{
			Contract.Requires(node != null);
			Debug.Assert(node.List == ResourceList);
		...
		/// <summary>
		/// 指定したノードが保持するリソースを完全に解放する。
		/// 指定したノードはキャッシュリストから削除して、DisposeResource メソッドを呼び出す。
		/// </summary>
		/// <param name="node"></param>
		protected void ReleaseCompletely(LinkedListNode<ResourceItem<WeakReference>> node)
		{
			Contract.Requires(node != null);
		...
		/// <param name="rc"></param>
		/// <returns></returns>
		protected LinkedListNode<ResourceItem<TResource>> AddNode(TKey key, TResource rc)
		{
			...
			var node = ResourceList.AddFirst(new ResourceItem<TResource>(key, rc));
			...
			// キャッシュリストから要素を探索
			{
				...
				if (node != null)
				{
					...
					if (node.Value.Resource.IsAlive)
					{
						...
						var newNode = ResourceList.AddFirst(new ResourceItem<TResource>(key, rc.Resource));
		...
		public void Clear()
		{
			...
			foreach (var item in resourceList)
			{
				...
				RemovingResource(rc);
				// キャッシュリストに追加
				var node = weakResourceList.AddFirst(new ResourceItem<WeakReference>
					(key, new WeakReference(new DisposableResource(this, key, rc))));
		...
		/// <summary>
		/// WeakReference で管理するために IDisposable を実装した
		/// リソースとリソースのキーの組合せを表します。
		/// </summary>
		protected class DisposableResource : IDisposable
		{
			...
			internal DisposableResource(ResourceManager<TKey, TResource> man, TKey key, TResource resource)
			{
				_man = man;
				_key = key;
				Resource = resource;
