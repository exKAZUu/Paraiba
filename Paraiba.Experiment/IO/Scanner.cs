...
{
	/// <summary>
	/// Java の Scanner を真似て作成したクラスです。
	/// ただし、Java版とは異なり、正規表現に対応していません。
	/// </summary>
	public class Scanner
	{
		...
		private Func<char, bool> _isWhiteSpace, _isDelimiter;
		...
		public Scanner(TextReader reader, ICollection<char> whiteSpaces, ICollection<char> delimiters)
			: this(reader, whiteSpaces.Contains, delimiters.Contains)
		{
		}

		public Scanner(TextReader reader, IEnumerable<char> whiteSpaces, IEnumerable<char> delimiters)
			: this(reader, whiteSpaces.Contains, delimiters.Contains)
		{
		}

		public Scanner(TextReader reader, Func<char, bool> isWhiteSpace, Func<char, bool> isDelimiter)
		{
			_reader = reader;
		...
		private bool ReadNextToken()
		{
			Int32 c;
			// 1文字目がEOFの場合、入力完了とする
			if ((c = _reader.Read()) < 0) return false;
		...
		public bool HasNextByte(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			Byte value;
			if (!Byte.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextUInt16(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			UInt16 value;
			if (!UInt16.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextUInt32(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			UInt32 value;
			if (!UInt32.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextUInt64(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			UInt64 value;
			if (!UInt64.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextSByte(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			SByte value;
			if (!SByte.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextInt16(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			Int16 value;
			if (!Int16.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextInt32(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			Int32 value;
			if (!Int32.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextInt64(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			Int64 value;
			if (!Int64.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}

		public bool HasNextSingle(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// 次のトークンの読み込み
			if (_lastToken == null && !ReadNextToken())
				return false;
			// 変換
			Single value;
			if (!Single.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}

		public bool HasNextDouble(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// 次のトークンの読み込み
			if (_lastToken == null && !ReadNextToken())
				return false;
			// 変換
			Double value;
			if (!Double.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextDecimal(NumberStyles style = NumberStyles.Number)
		{
			...
			// 変換
			Decimal value;
			if (!Decimal.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public bool HasNextBigInteger(NumberStyles style = NumberStyles.Integer)
		{
			...
			// 変換
			BigInteger value;
			if (!BigInteger.TryParse(_lastToken, style, NumberFormatInfo.CurrentInfo, out value))
				return false;
			_cache = value;
			return true;
		}
		...
		public Single CurrentSingle(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// キャッシュのチェック
			if (_lastToken == null)
			{
		...
		public Double CurrentDouble(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// キャッシュのチェック
			if (_lastToken == null)
			{
		...
		public BigInteger CurrentBigInteger(NumberStyles style = NumberStyles.Integer)
		{
			// キャッシュのチェック
			if (_lastToken == null) {
				if (!ReadNextToken())
		...
		public Single NextSingle(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// キャッシュのチェック
			if (_lastToken == null)
			{
		...
		public Double NextDouble(NumberStyles style = NumberStyles.Float | NumberStyles.AllowThousands)
		{
			// キャッシュのチェック
			if (_lastToken == null)
			{
		...
		public BigInteger NextBigInteger(NumberStyles style = NumberStyles.Integer)
		{
			// キャッシュのチェック
			if (_lastToken == null) {
				if (!ReadNextToken())
