...
namespace Paraiba.Collections.Generic
{
    ...
    {
		public static bool SequenceEqual<T>(this IList<T> list, IEnumerable<T> that, int startIndex = 0)
		{
			return that.All(item => startIndex < list.Count && list[startIndex++].Equals(item))
				&& startIndex == list.Count;
		}

		public static bool IsSubSequence<T>(this ICollection<T> collection, IList<T> parentSequence)
		{
		...
		/// <summary>リストに指定した要素を追加して、有効要素数を拡張する</summary>
        ...
        public static List<T> Extend<T>(this List<T> list, int count, T defaultElement)
        {
        ...
        public static IEnumerable<T> GetEnumerable<T>(this IList<T> list, int startIndex)
        {
        ...
        public static IEnumerable<IList<T>> GetPermutations<T>(this IList<T> list, IList<T> outList)
        {
            return list.GetPermutations(list.Count, outList);
        }

        public static IEnumerable<IList<T>> GetPermutations<T>(this IList<T> list, int N)
        {
            var array = new T[N];
            for (int i = 0; i < N; i++)
                array[i] = list[i];
        ...
        public static IEnumerable<IList<T>> GetPermutations<T>(this IList<T> list, int N, IList<T> outList)
        {
            for (int i = 0; i < N; i++)
                outList[i] = list[i];
        ...
        public static IEnumerable<IList<T>> GetPermutationsDestructively<T>(this IList<T> list)
        {
            return list.GetPermutationsDestructively(list.Count);
        }

        public static IEnumerable<IList<T>> GetPermutationsDestructively<T>(this IList<T> list, int N)
        {
            // 階乗進法でN!を表現する
        ...
        public static int ForEachDestructively<T>(this IList<T> list, Func<T, T> func)
        {
        ...
        public static IList<T> StableSort<T>(this IList<T> list, IComparer<T> cmp)
        {
            return null;
        }
