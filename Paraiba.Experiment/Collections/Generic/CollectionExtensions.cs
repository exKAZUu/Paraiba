...
namespace Paraiba.Collections.Generic
{
    public static class CollectionExtensions
    {
        ...
        /// <summary>コレクションに指定した要素を追加して、有効要素数を拡張する</summary>
		/// <param name="collection">拡張するコレクション</param>
        /// <param name="count">拡張するコレクションのサイズ</param>
		public static ICollection<T> Extend<T>(this ICollection<T> collection, int count)
        {
        ...
        public static ICollection<T> Extend<T>(this ICollection<T> collection, int count, T defaultElement)
        {
        ...
        public static ICollection<T> Extend<T>(this ICollection<T> collection, int count, Func<T> defaultElementFunc)
        {
            for (int i = collection.Count; i < count; i++)
        ...
        public static TResult[] SelectToArray<TSource, TResult>(
            this ICollection<TSource> collection, Func<TSource, TResult> func)
        {
        ...
        public static TResult[] SelectToArray<TSource, TResult>(
            this ICollection<TSource> collection, Func<TSource, int, TResult> func)
        {
            var array = new TResult[collection.Count];
            var i = 0;
            foreach (var item in collection)
